...
Using the Linear connector, list issues on team `SAU` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `SAU-NNN-short-slug`, using the issue's real identifier.
...
- `Closes SAU-NNN`, using the real Linear issue identifier
